<?php

namespace App\Domains\School\Repositories\Contracts;

use App\Domains\School\Models\SchoolClass;
use App\Domains\Identity\Models\User;
use Illuminate\Database\Eloquent\Collection;

interface ScheduleRepositoryInterface
{
    public function getByClass(SchoolClass $schoolClass): Collection;
    public function getByTeacher(User $user): Collection;
    public function getTeacherClasses(User $user): Collection;
    public function create(array $data): \App\Domains\School\Models\Schedule;
    public function update(int $scheduleId, array $data): \App\Domains\School\Models\Schedule;
    public function delete(int $scheduleId): bool;
    // tim ban da xoa mem
    public function findTrashed(int $scheduleId): ?\App\Domains\School\Models\Schedule;
    // restore lai ben soft delete
    public function restore(int $scheduleId): bool;
}
